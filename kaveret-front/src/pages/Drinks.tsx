...
const Drinks = () => {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', paddingTop: '5rem', paddingRight:"15rem"}} dir='rtl'>
        <Typography variant="h2" style={{paddingLeft:"80rem", paddingRight:"5rem"}}>שתיה קלה</Typography>

      {items
        .filter((item) => item.Category === 'Drinks')
        .map((item, index) => (
          <Card key={index} style={{ margin: '1rem', padding: '1rem', boxShadow: '0 4px 8px 0 rgba(0,0,0,0.2)', transition: '0.3s', borderRadius: '4px', height:"24rem", width:"15rem"}}>
            <CardContent>

              <CardMedia component="img" alt={item.Name} height="100" style={{ height: '12rem', width: '100%', alignSelf: 'center' }} image={item.ImageUrl} />
